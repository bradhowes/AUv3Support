...
public:
   ...
   */
  LFO(Parameters::Float& frequency, ValueType sampleRate, ValueType frequency, LFOWaveform waveform) noexcept
  : valueGenerator_{WaveformGenerator(waveform)}, waveform_{waveform}, sampleRate_{sampleRate},
  phaseIncrement_ {address} {
  ...
  /**
   ...
   @param frequency the frequency of the oscillator
   */
  LFO(AUParameterAddress address, ValueType sampleRate, ValueType frequency) noexcept
  : LFO(address, sampleRate, frequency, LFOWaveform::sinusoid) {}

  /// Create a new instance.
  LFO(AUParameterAddress address) noexcept : LFO(address, 44100.0, 1.0, LFOWaveform::sinusoid) {}
